...
function isValidEntry(entry: unknown): entry is ChatAllowlistEntry {
  ...
  const validAllow =
    candidate.allow === "*" ||
    (Array.isArray(candidate.allow) && candidate.allow.every((value) => typeof value === "string"));
  const validMode = candidate.mode === "trigger" || candidate.mode === "drop";
...
export function loadSenderAllowlist(pathOverride?: string): SenderAllowlistConfig {
  ...
  try {
    ...
    logger.warn({ error, path: filePath }, "sender-allowlist: cannot read config");
    return DEFAULT_CONFIG;
  }
  ...
  if (!isValidEntry(object.default)) {
    logger.warn({ path: filePath }, "sender-allowlist: invalid or missing default entry");
    return DEFAULT_CONFIG;
  }

  const chats: Record<string, ChatAllowlistEntry> = {};
  if (object.chats && typeof object.chats === "object") {
    for (const [jid, entry] of Object.entries(object.chats as Record<string, unknown>)) {
  ...
  return {
    default: object.default as ChatAllowlistEntry,
    chats,
    logDenied: object.logDenied !== false
  };
}
